...
public class Juego {
  ...
  public String obtenerTiempoTotal() {
    ...
    String tiempoTotalFormato = String.format(
      "%02d:%02d:%02d",
      horas,
      minutos,
      segundos
  ...
  public void crearTableroAleatorio(int filas, int columnas, int nivel) {
    ...
    } while (Tablero.verificarTablero(t.getTableritoActual()) || solucionarTablero(t, historiales.getSolucion())); // evitar que el tablero sea todo de un color o se solucione antes
    
    this.tablero = t;
  }
  
  private boolean solucionarTablero(Tablero t, ArrayList<Movimiento> solucion) {
      String[][] tableroNuevo = copiarTablero(t.getTableritoActual());
      int penultimoMovimiento = solucion.size() - 1;
      boolean seSolucionaAntes = false;
      
      for(int i = 0; i < penultimoMovimiento; i++) {
          Movimiento m = solucion.get(i);
          int posicionFila = m.getFila() - 1;
          int posicionColumna = m.getColumna() - 1;
    
          String celda = tableroNuevo[posicionFila][posicionColumna];
          
          aplicarMovimiento(celda, posicionFila, posicionColumna, tableroNuevo);
          
          if(Tablero.verificarTablero(tableroNuevo)) {
              seSolucionaAntes = true;
          }
      }
      
      return seSolucionaAntes;
  }
  ...
  public boolean jugar(int fila, int columna) {
    ...
    boolean filaValida =
      posicionFila >= 0 &&
      posicionFila < this.tablero.getTableritoActual().length;
    boolean columnaValida =
      posicionColumna >= 0 &&
      posicionColumna < this.tablero.getTableritoActual()[0].length;
    boolean FilaColumnaRetroceso = fila == -1 && columna == -1;
    boolean tableroCompletado = Tablero.verificarTablero(this.tablero.getTableritoActual());
      ...
      if (filaValida && columnaValida || FilaColumnaRetroceso) {
        if (!FilaColumnaRetroceso) {
          ...
          if (!historiales.obtenerMovimientos().isEmpty()) {
            Movimiento ultimoMovimiento = historiales
              .obtenerMovimientos()
              .get(historiales.obtenerMovimientos().size() - 1);
            String celda =
              tableroNuevo[ultimoMovimiento.getFila() -
                1][ultimoMovimiento.getColumna() - 1];
            ...
            throw new RuntimeException(
              "Este es el tablero inicial, no se puede aplicar paso de retroceso."
            );
          }
        }

        historiales.guardarMovimiento(fila, columna);
        tableroCompletado = Tablero.verificarTablero(this.tablero.getTableritoActual());
        this.tiempoFin = System.currentTimeMillis();
      } else {
    ...
    switch (simbolo) {
      case '|':
        for (int i = 0; i < tableroNuevo.length; i++) {
          ...
          tableroNuevo[i][columna] =
            simboloActual + cambiarColorOpuesto(colorActual);
        }
        break;
      case '-':
  ...
  private boolean aplicarMovimientoEnCelda(
    Tablero tablero,
    int fila,
    int columna
  ) {
  ...
  private boolean existenMovimientos(int fila, int columna) {
    ...
    for (Movimiento unMovimiento : historiales.getSolucion()) {
      if (
        unMovimiento.getColumna() == columna && unMovimiento.getFila() == fila
      ) {
        existe = true;
      }
    }
